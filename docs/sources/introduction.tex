...
\mono{item|...}&  one or more items, bar-separated\\
\mono{[]} &optional\\
\bottomrule
\\
\end{tabularx}
...
\end{code} 
 ...
 Almost nothing need be resolved in the first pass.	\dasm is most likely to make multiple passes through the source code to resolve all symbols.  The maximum number of passes (default 10) is controllable by \nameref{flag:passes} and \nameref{flag:passes2}.  \dasm will return an error if it can't resolve all referenced symbols within the maximum number of passes.
