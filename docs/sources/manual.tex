...
This example will assemble the file \mono{source.asm}, using output format \mono{2} (random access segments). The resultant binary will be saved to the file \mono{output.bin} and a listing file written to \mono{listing.txt}. During the assembly, verbosity of the output is set to \mono{3} (unresolved and unreferenced symbols displayed every pass). The value of the symbol (which will be available in the source code) \mono{VER} is set to \mono{4}.
...
\index{Base!2 binary}
\index{Base!8 octal}
\index{Base!10 decimal}
\index{Base!16 hexadecimal}
...
\begin{code}
 ...
 lda #%100000001    ; ERROR - only 8 bits in binary bytes
...
Square brackets \mono{[]} may be used to group expressions.  The precedence of operators
is the same as for the C-language in almost all respects.  Use
brackets \mono{[ ]} when you are unsure. The reason round brackets \mono{()} cannot be used to
group expressions is due to a conflict with 6502 and other assembly languages which use them to specifiy indirect memory access (for example, ``\mono{lda (zp),y}'').
...
{\color{bug}Bug: Currently \dasm allows values in the range \mono{-\$FF} to \mono{+\$FF}. This is incorrect. The correct range is \mono{-\$80} to \mono{+\$FF}}.
...
\hline
\\
\mono{6502}      &MOS Technology 6502   &little-endian  &\mono{LSB}, \mono{MSB}\\
\mono{68HC11}    &Motorola 68HC11       &big-endian     &\mono{MSB}, \mono{LSB}\\
\mono{68705}     &Motorola 68705        &big-endian     &\mono{MSB}, \mono{LSB}\\
\mono{6803}      &Motorola 6803         &big-endian     &\mono{MSB}, \mono{LSB}\\
\mono{HD6303}    &Hitachi HD6303        &big-endian     &\mono{MSB}, \mono{LSB}\\
\mono{F8}        &Fairchild F8          &big-endian     &\mono{MSB}, \mono{LSB}\\
\\
\bottomrule
\end{tabularx}
\end{table}
