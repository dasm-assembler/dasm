...
A comment directive anywhere on a line ("\mono{;}" or "\mono{/*}" or "\mono{*/}") terminates \dasm's label/directive/instruction parser for the rest of that line.
...
\subsubsection{Malformed Examples}
\begin{code}
...
*/ a closing comment directive without a starting one first
   is flagged by dasm as an error

/* This comment is missing the closing end-of-comment.
   dasm will flag an error
...
\begin{outputx2}
> dasm
boo@Andrews-MacBook-Pro dasm % bin/dasm
...
\label{flag:maximum}
\index{Command Line!Options!Maximum File Size}
\begin{usage}
-mvalue
\end{usage}

\label{changelog:20200908optionm}
Sets the maximum size (in \mono{KiB} units) allowed for the output binary. Can be used in conjunction with \nameref{flag:remove} to detect assembly anomalies and clean-up afterwards.

\subsubsection{Example}
\begin{outputx}
> dasm source.asm -orom.bin -m2
\end{outputx}

This example will assemble the file \mono{source.asm} and write the file \mono{rom.bin}, restricting the output binary to \mono{2 KiB}. An error will be generated if the output file exceeds this size. This can be useful for ensuring ROM size limits are not exceeded.
...
\label{flag:remove}
\index{Command Line!Options!Remove}
\begin{usage}
-R
\end{usage}
