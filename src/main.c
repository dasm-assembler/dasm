...
static void ShowSegments(void) {
    ...
    for (seg = Seglist; seg; seg = seg->next) {
        ...
        printf(sFormat, seg->name, bss, sftos(seg->initorg, seg->initflags), sftos(seg->initrorg, seg->initrflags),
...
static int MainShadow(int ac, char **av, int *pSortMode) {
    ...
    char buf[MAXLINE * 2]; /* needs room for comment reformatting in cleanup() */
    int i;
    long argVal;
    MNEMONIC *mne;
    ...
    for (i = 2; i < ac; ++i) {
        if ((av[i][0] == '-') || (av[i][0] == '/')) {
            ...
            case 'T':
                F_sortmode = atoi(str);
                if (F_sortmode < SORTMODE_DEFAULT || F_sortmode >= SORTMODE_MAX) {
            ...
            case 'p': /*  F_passes   */
                nMaxPasses = atoi(str);
                if (nMaxPasses < 1)
                    panic("-p Switch requires a positive pass count.");
                break;
    ...
    if (bStopAtEnd == NULL)
        panic("unable to allocate pass-stop array");
    memset(bStopAtEnd, 0, (nMaxPasses + 1) * sizeof(bool)); /* Bug: was missing sizeof(bool) */
...
nextpass:
    ...
    if (F_listfile) {
        ...
        FI_listfile = fopen(F_listfile, F_ListAllPasses && (pass > 1) ? "ab" : "wb");
...
static void outlistfile(const char *comment) {
    ...
    snprintf(buf1 + j - 1, sizeof(buf1) - (j - 1), "%c%-10s %s%s%s\t%s\n", xtrue, Av[0], Av[1], dot, ptr, Av[2]);
    if (comment[0]) { /*  tab and comment */
        j = strlen(buf1) - 1;
        snprintf(buf1 + j, sizeof(buf1) - j, "\t;%s", comment);
    }
            ...
            if (mlstart && ((!semistart) || (mlstart < semistart))) {
                if (mlend && (mlstart < mlend) && ((!semistart) || (mlend < semistart)))    // single line /* */
                {
                    ...
                    *(mlend + 1) = 0;
                    ...
                    snprintf(tempbuf, MAXLINE * 2, "%s%s;/%s", buf, mlend + 2,
                             mlstart + 1);    // move the first comment to the end of the line
                    strcpy(buf, tempbuf);
                    continue;    // reprocess buf for any remaining comments
                }
...
/* Bounds-checked write into Avbuf — panics on overflow rather than overwriting stack */
#define AVBUF_WRITE(ch)                                                                                                \
    do {                                                                                                               \
        if (j >= (int)(MAXLINE * 3) - 1)                                                                               \
            panic("source line too long (parse buffer overflow)");                                                     \
        Avbuf[j++] = (ch);                                                                                             \
    } while (0)
    ...
    */
            ...
            } else    // or else it's a symbol to be evaluated, and added to the label
            {
                ...
                if (symarg) {
                    if (symarg->flags & SYM_UNKNOWN)    // one of the arguments isn't defined yet
                        ...
                        vlen = strlen(tempval);
                        if (j + (int)vlen >= (int)(MAXLINE * 3) - 1)
                            panic("source line too long (label argument overflow)");
    ...
    Avbuf[j] = 0; /* final NUL — no increment */
#undef AVBUF_WRITE
...
void v_macro(char *str, MNEMONIC *dummy) {
    ...
    while (fgets(buf, MAXLINE, pIncfile->fi)) {
        ...
        mne = parse(buf);
        if (Av[1][0]) {
            if (mne != NULL) {    // for some reason I got a segfault while accessing mne->flags, should not happen but
                                  // gdb said it was here
                if (mne->flags & MF_ENDM) {
                    if (!defined)
                        mac->strlist = base;
                    return;
...
int asmerr(int err, bool bAbort, const char *sText) {
    ...
    if (err >= MAX_ERROR || err < 0) {
        ...
        while (1) {
            if (pincfile == NULL) {
                fprintf(stderr, "%s:%d: error: pincfile is NULL, err:%d, [%s]: %s\n", __FILE__, __LINE__, err, sText,
                        sErrorDef[err].sDescription);
                bAbort = true;
                break;
            }
            if (pincfile->flags & INF_MACRO) {
        ...
        */
        ...
        if (pincfile != NULL)
            ...
            case ERRORFORMAT_WOE:
                ...
                */
                if (error_file != stdout)
                    fprintf(error_file, "%s (%lu): error: ", pincfile->name, pincfile->lineno);
                snprintf(erroradd1, sizeof(erroradd1), "%s (%lu): error: ", pincfile->name,
                         pincfile->lineno);    // Bug K fix: was sprintf, overflow on long filenames
                break;
            case ERRORFORMAT_DILLON:
                ...
                */
                if (error_file != stdout)
                    fprintf(error_file, "line %7ld %-10s ", pincfile->lineno, pincfile->name);
                snprintf(erroradd1, sizeof(erroradd1), "line %7ld %-10s ", pincfile->lineno,
                         pincfile->name);    // Bug K fix: was sprintf, overflow on long filenames
                break;
        ...
        /* Format the message first, then print — avoids passing str as a non-literal format string */
        snprintf(erroradd2, sizeof(erroradd2), str,
                 sText ? sText
                       : ""); /* Bug J fix: was sprintf, overflow on long sText (labels/filenames up to 1024 chars) */
...
void passbuffer_update(int mbindex, char *message) {
    ...
    size_t newsizerequired;

    // allocate 16k buffers to start...
    static size_t passbuffersize[2] = {16384, 16384};
    ...
    if (newsizerequired > passbuffersize[mbindex]) {
        ...
        newsize = (newsizerequired < passbuffersize[mbindex] * 2) ? passbuffersize[mbindex] * 2 : newsizerequired;

        tmpalloc = realloc(passbuffer[mbindex], newsize);
        if (tmpalloc == NULL) {
            /* Can't grow — report to stderr and drop the message rather than corrupting the buffer */
            fputs("dasm: warning: insufficient memory to extend pass buffer; some output was lost.\n", stderr);
            return;
        }
        passbuffer[mbindex] = tmpalloc;
        passbuffersize[mbindex] = newsize;
    }
