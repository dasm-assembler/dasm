...
void v_mnemonic(char *str, MNEMONIC *mne) {
  ...
  if (bTrace)
    printf("PC: %04lx  MNEMONIC: %s  addrmode: %d  ", Csegment->org, mne->name,
           symbase->addrmode);
  ...
  if (mne->opcode[addrmode] == 0x42 && addrmode == AM_INDBYTEX &&
      Processor == 16502)
    addrmode = AM_INDWORDX;
  else if (mne->opcode[addrmode] == 0x42 && addrmode == AM_INDBYTE &&
           Processor == 16502)
    addrmode = AM_INDWORD;
  ...
  case AM_INDWORD:
  case AM_WORDADR_SP:
    if (bStrictMode && ((sym->value > 0xFFFF) ||
                        (sym->value < -0xFFFF))) { // isn't this our space ?
      sprintf(sBuffer, "%s %s", mne->name, str);
...
void v_trace(char *str, MNEMONIC *dummy) { bTrace = (str[1] == 'n'); }
...
void v_set(char *str, MNEMONIC *dummy) {
  ...
  if (str[i] &&
      str[i] == ',') // is this SET is using the "," eval-concat operator?
  {
    ...
    while (str[i] && str[i] != '"' && str[i] != ' ') {
      if (str[i] == 0 ||
          str[i] == ' ') // leave if we've processed all arguments
      {
        break;
      }
      ...
      else // this argument is a symbol to be evaluated
      {
        ...
        if (symarg) {
          if (symarg->flags &
              SYM_UNKNOWN)  // one of the arguments isn't defined yet
...
void v_mexit(char *str, MNEMONIC *dummy) { v_endm(NULL, NULL); }
...
void generate(void) {
  ...
  if (Csegment->org > maxFileSize) {
    ...
    snprintf(errMsg, sizeof(errMsg),
        "code segment growing larger (%ld) than max. allowed file size (%ld)",
        Csegment->org, maxFileSize);
    asmerr(ERROR_AVOID_SEGFAULT, true, errMsg); /* Bug L fix: was ERROR_RECURSION_TOO_DEEP, wrong code for size-exceeded */
    return;
  }
}
